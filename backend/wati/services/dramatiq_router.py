...
#     now = datetime.utcnow()  # Make now timezone-naive
...
#     # Update the broadcast list entry with the task ID
...
async def schedule_template_message(
    ...
    task: Message = send_broadcast.send_with_options(
        args=[request.template, contacts, saved_broadcast_id, API_url, headers, get_current_user.id, request.image_id, request.body_parameters,get_current_user.Phone_id],
